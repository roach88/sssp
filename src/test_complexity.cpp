...
int main() {
    std::cout << "Testing Constant-Degree Transformation Complexity Bounds\n";
    std::cout << "=========================================================\n";
    ...
    // Test 1: Small complete graph
    {
        ...
        assert(analysis.maintains_linear_bound);
        std::cout << "  ✓ Test passed\n";
    }
    
    // Test 2: Star graph with high-degree center
    {
    ...
    // Test 3: Random graph with varying degrees
    {
    ...
    // Test 4: Test optimized transformation
    {
        ...
        assert(opt_analysis.transformed_vertices <= std_analysis.transformed_vertices);
        assert(opt_analysis.maintains_linear_bound);
        
        // Verify both maintain degree constraints
        for (const auto& v : optimized.vertices()) {
            assert(optimized.in_degree(v) <= 2);
            assert(optimized.out_degree(v) <= 2);
        }
        
        std::cout << "  ✓ Optimization test passed\n";
    }
    
    // Test 5: Large graph stress test
    {
        ...
        assert(transformed.num_vertices() <= 3 * m);
        assert(transformed.num_edges() <= 3 * m);
        
        std::cout << "  Upper bound check: ";
        std::cout << transformed.num_vertices() << " ≤ " << (3 * m) << " (3m) ✓\n";
        std::cout << "  ✓ Large graph test passed\n";
    }
    ...
    std::cout << "\n==========================================================\n";
    std::cout << "All complexity tests passed! ✓\n";
    std::cout << "The transformation maintains O(m) vertices and edges.\n";
